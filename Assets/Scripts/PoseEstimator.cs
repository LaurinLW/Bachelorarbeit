...
public class PoseEstimator : MonoBehaviour
{
    ...
    [Tooltip("Scale the webcam in the game")]
    [Range(0, 1.0f)]
    ...
    void Update()
    {
        ...
        float localScale = (float)minDimension / Mathf.Min(imageDims.x, imageDims.y);
        ...
        for (int i = 0; i < skeletons.Length; i++)
        {
            if (i <= poses.Length - 1)
            {
                ...
                skeletons[i].UpdateKeyPointPositions(poses[i], localScale, videoTexture, useWebcam, minConfidence, scale);
